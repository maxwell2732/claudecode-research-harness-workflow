...
description: "Read and understand repository context (README, Plans.md, existing code). Use at session start, before beginning a new task, or when you need to understand the project structure."
...
- **Optional**: A specified focus on particular files or directories
...
1. **At session start**: Understand current state
2. **Before implementing a new feature**: Verify alignment with existing code
...
- **Large repositories**: For many files, focus on the important parts
- **Sensitive information**: Do not read contents of .env or secrets/
- **Leverage caching**: Minimize re-reading within the same session
